FUNCTION zmfmm_mcte_conf_pag_post .
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
...
* Constantes Locais
  ...
  SELECT SINGLE multiid, status_proc
    FROM ztmm_028
    INTO @DATA(wl_ztmm_028)
    WHERE multiid EQ @i_post-multiid.

  MOVE: i_post-multiid           TO wl_conf_pag-protocolocte,
        i_post-data              TO wl_conf_pag-datapagamento,
        i_post-observacao        TO wl_conf_pag-observacao.
...
* SM59 Configuração
  ...
  IF ol_http_client IS NOT INITIAL AND wl_ztmm_032-url IS NOT INITIAL
     AND wl_ztmm_034 IS NOT INITIAL AND wl_ztmm_028-multiid IS NOT INITIAL.
...
* ABAP to JSON
      MOVE-CORRESPONDING wl_conf_pag TO wl_json_req.

      CALL FUNCTION 'CONVERT_DATE_TO_EXTERNAL'
        EXPORTING
          date_internal            = wl_conf_pag-datapagamento
        IMPORTING
          date_external            = wl_json_req-datapagamento
        EXCEPTIONS
          date_internal_is_invalid = 1
          OTHERS                   = 2.

      REPLACE ALL OCCURRENCES OF '.' IN wl_json_req-datapagamento WITH '/'.
...
* Set Payload or body ( JSON or XML)
      ...
      REPLACE ALL OCCURRENCES OF 'datapagamento' IN vl_body WITH 'dataPagamento'.
